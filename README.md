...
Trilinos must be built with the same MPI build used by the system binary configured through `MPIPreferences`.
...
sbatch slrum_jobs/<job-script-to-run-buildcpp.sh>.sh
```
...
sbatch slrum_jobs/<job-script-to-run-build_sysimage.sh>.sh
```
...
## Write your own jobs script for SLRUM workflow

Use `example_job.sh` as a template and write your job script in the `slrum_jobs` dir. 
...
```bash
sbatch slrum_jobs/<job-script>.sh
```
