@with_kw struct ToyRichardsonParams
  # Case metadata
  case::String = "test"
...
function toyrichardson(ω::Float64, params::ToyRichardsonParams)

  # Unpack the parameters
  @unpack_ToyRichardsonParams params
  ...
  V = TestFESpace(
    Ωf,
    reffeᵤ,
    dirichlet_tags = ["Bottom", "BCMembrane"],
  ...
  S = TestFESpace(
    Ωs,
    reffeₛ,
    dirichlet_tags = ["!Membrane", "BCMembrane"],
    conformity = :H1,
  )
  ...
  jac_t(t, (dtu, dtp, dtd), (v, q, s)) = (
    ∫(ρf * v ⋅ dtu)dΩf +
    ∫(ρf * s ⋅ (dtu ⋅ nᵥ))dΩs -
    ∫(αf * (dtd * (v ⋅ nᵥ)))dΣ -
    ∫(-αs * (dtd * s))dΣ
  )
  jac_tt(t, (dttu, dttp, dttd), (v, q, s)) = (
    ∫(ρs * hs * (v ⋅ nᵥ) * dttd)dΣ +
    ∫(ρs * hs * s * dttd)dΣ
  )
  l(t, (v, q, s)) = (
    ∫(v ⋅ VectorValue(0.0, 0.0))dΩf +
    ∫(q * 0.0)dΩf +
    ∫(s * 0.0)dΣ -
    ∫(gpl(t) * v ⋅ (-nᵤ))dΣleft -
    ∫(gpr(t) * v ⋅ nᵤ)dΣright
  ...
  get_ones_scalar(t) = x -> 1.0
  get_ones_vector(t) = x -> VectorValue(1.0, 1.0)
  get_relax_scalars(t) = x -> ω
  ω_vec = get_free_dof_values(
    interpolate_everywhere(
      [get_ones_vector(t0), get_ones_scalar(t0), get_relax_scalars(t0)],
      X(t0),
    ),
  )
  ...
  @vtk pvd_Ωf[0] = createvtk(
    Ωf,
    "$(vtk_dir)/results_Fluids_in_2D_0",
    cellfields = ["u" => x_initial[1], "pressure" => x_initial[2]],
  )
  @vtk pvd_Σ[0] = createvtk(
    Σ,
    "$(vtk_dir)/results_Solids_in_2D_0",
    cellfields = ["displacement" => x_initial[3]],
  )
  ...
  for (tn, (uhn, phn, dhn)) in xₜ
    @vtk pvd_Ωf[tn] = createvtk(
      Ωf,
      "$(vtk_dir)/results_Fluids_in_2D_$tn",
      cellfields = ["u" => uhn, "pressure" => phn],
    )
    @vtk pvd_Σ[tn] = createvtk(
      Σ,
      "$(vtk_dir)/results_Solids_in_2D_$tn",
