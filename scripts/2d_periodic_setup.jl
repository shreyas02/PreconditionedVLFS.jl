...
function warmup()
    with_mpi() do distribute
        ...
        _ = periodic2D(distribute, parts, case)
    end
end
...
function strong_scaling()
    ...
    with_mpi() do distribute
        ...
        path = datadir("periodic2d","strong_scaling")
        ...
        if i_am_main(ranks)
            ...
            savefig(fig, plotsdir("periodic2d_strong_scaling_first_solve.png"))
            ...
            savefig(fig, plotsdir("periodic2d_strong_scaling_subsequent_solves.png"))
        end
    end
end
...
function weak_scaling()
    ...
    with_mpi() do distribute
        ...
        runtime_seconds(timer,field) = TimerOutputs.time(timer["Time Stepping Loop Aggregate"][field]) / 1e9
        path = datadir("periodic2d","weak_scaling")
        df = collect_results(path)::DataFrame
        ...
        if i_am_main(ranks)
            ...
            fig = plot(p1, p2; layout = (1, 2))
            savefig(fig, plotsdir("periodic2d_weak_scaling_first_solve.png"))
            ...
            fig = plot(p3, p4; layout = (1, 2))
            savefig(fig, plotsdir("periodic2d_weak_scaling_subsequent_solves.png"))
        end
    end
end
